...
(function() {
    ...
    var bannerDataFromTabId = function(tabId, tabTitle) {
        let tabContext = µb.tabContextManager.mustLookup(tabId),
            rootHostname = tabContext.rootHostname;
    ...
    var bannerDataFromRequest = function(request, callback) {
        if (request.tabId) {
            callback(bannerDataFromTabId(request.tabId, ''));
            return;
        }
        ...
        vAPI.tabs.get(null, function(tab) {
            ...
            callback(bannerDataFromTabId(tabId, tabTitle));
        });
    };
    ...
    var onMessage = function(request, sender, callback) {
        ...
        switch (request.what) {
            case 'getPopupData':
                ...
                return;
            case 'getBannerData':
                bannerDataFromRequest(request, callback);
                return;
            default:
                break;
        }
    ...
    vAPI.messaging.listen('banner', onMessage);
...
(function() {
    ...
    let setShortcut = function(details) {
        ...
        vAPI.storage.set({
            commandShortcuts: Array.from(µb.commandShortcuts)
        });
    };
            ...
            browser.tabs.query({
                ...
                tabs => {
                    ...
                    browser.windows.get(tabs[0].windowId, win => {
                        if (win instanceof Object === false) {
                            return;
                        }
